...
void GPTree::init_dist_map() {
	...
	dist_map = hps::from_stream<std::vector<pair<unsigned short int, unsigned int>>>(in_file);
	int nCombos = max_node * (max_node - 1) / 2;
	if (dist_map.size() < nCombos) {
		dist_map.clear();
		dist_map.reserve(max_node * (max_node - 1) / 2);
		int i = 0;
	//#pragma omp parallel for default(none) private(i) shared(dist_map, outDir, logFile)
	for (i = 1; i <= max_node; i++) {
		for (int j = i + 1; j <= max_node; j++) {
			std::vector<int> order;
			dist_map.push_back(find_path(i - 1, j - 1, order));
			//if(j % 500 == 0){
			//	print_line(outDir, logFile, string_format("i=%d,j=%d", i,j));
			//}
		}
	}
