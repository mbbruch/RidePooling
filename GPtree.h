...
struct Graph// Undirected graph structure
{
	...
	friend void reinitialize_dist_map(std::set<std::set<int>>& ongoingLocs,
	...
	extern void reinitialize_dist_map(std::set<std::set<int>>& ongoingLocs,
		set<int>& allToAll1, set<int>& allToAll2,
		map_of_pairs& mop);
